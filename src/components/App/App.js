  ...
  const onAddToCart = (obj) => {
    ...
    try {
      if(cartItems.find((item) => Number(item.id) === Number(obj.id) )) {
        axios.delete(`https://60d51f31943aa600177687d3.mockapi.io/cart/${obj.id}`)
  ...
  const onAddToFavorite = async (obj) => {
    ...
    try {
      if(favorites.find(favObj => favObj.id === obj.id)) {
        axios.delete(`https://60d51f31943aa600177687d3.mockapi.io/favorites/${obj.id}`)
      } else {
        const { data } = await axios.post('https://60d51f31943aa600177687d3.mockapi.io/favorites',obj);
